...
function createTestFixture() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'engram-save-test-'));
...
function loadSessionSaverWithFixture(fixturePath) {
  ...
  return { SessionSaver, restore: () => { pathsModule.resolveEngramPath = originalResolve; } };
}
...
describe('SessionSaver', () => {
  ...
  describe('saveSession()', () => {
    it('saves session without git commit by default', async () => {
      const saver = Object.create(SessionSaver.prototype);
      saver.currentProject = 'TestSaveProject';
      saver.engram = require('../scripts/engram-loader');
    ...
    it('emits session saved event with git stats when bridge is configured', async () => {
      ...
      assert.equal(emitted[0].eventType, 'engram.session.saved');
