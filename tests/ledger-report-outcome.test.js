...
function makeTestEnv() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'memex-ro-'));
  const cacheDir = path.join(dir, '.cache');
  fs.mkdirSync(cacheDir);
  const db = new Database(path.join(cacheDir, 'memex.db'));
  db.pragma('foreign_keys = ON');
  runSqlMigrations(db);
  db.close();
  return { dir, dbPath: path.join(cacheDir, 'memex.db') };
}
...
test('returns ok with message when DB not found', async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'memex-empty-'));
  const savedPath = process.env.MEMEX_PATH;
  ...
  try {
    ...
    process.env.MEMEX_PATH = savedPath;
    clearRequireCache('mcp-tools', 'capture', 'feedback/post-hoc', 'feedback/score-citations');
