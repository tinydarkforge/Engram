...
#
# This hook:
# 1. Auto-updates Memex from Cirrus-Inc/Memex
...
# Auto-update Memex from Cirrus-Inc (non-blocking, background)
(
  cd "$MEMEX_PATH" 2>/dev/null || exit

  # Ensure cirrus remote exists
  if ! git remote | grep -q "^cirrus$"; then
    git remote add cirrus https://github.com/Cirrus-Inc/Memex.git 2>/dev/null
  fi
